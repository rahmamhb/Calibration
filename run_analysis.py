...
#
# Usage:
#   python3 run_analysis.py --dir ./results/cooja --nodes 3
# =============================================================================
...
def parse_log(serial_path):
        ...
        for line in f:
            ...
            # ── Receive event ─────────────────────────────────────────────
                ...
                if rssi_match:
                    ...
                    recv_by_node[label] = recv_by_node.get(label, 0) + 1
    ...
    for send_time, recv_time in packet_dict.values():
        if send_time is not None and recv_time is not None:
            ...
            if r >= s:   # >= keeps zero-delay packets, matching compute_delay and kpi_extractor
                delay_list.append(r - s)

    duration = last_time - starting_time
    return packet_dict, rssi_by_node, duration, all_rssi, delay_list, sent_by_node, recv_by_node
...
def compute_lossrate(packet_dict):
    total = sum(1 for s, _ in packet_dict.values() if s is not None)
    lost  = sum(1 for s, r in packet_dict.values() if r is None)
    return (lost / total * 100) if total else 0.0


def compute_throughput(packet_dict, duration, nb_nodes, msg_size=8):
    received = sum(
        1 for s, r in packet_dict.values()
...
if __name__ == '__main__':
    ...
    packet_dict, rssi_by_node, duration, all_rssi, delay_list, sent_by_node, recv_by_node = parse_log(serial_path)

    delay      = compute_delay(packet_dict)
    delay_std  = compute_delay_std(delay_list)                          # NEW
    lossrate   = compute_lossrate(packet_dict)
    # Normalize by nodes that actually sent, not the --nodes CLI value — keeps
    # this consistent with kpi_extractor.py's live/adaptive-monitor calculation.
    active_senders = len(sent_by_node) or args.nodes
    throughput     = compute_throughput(packet_dict, duration, active_senders)
    ...
    if rssi_means:
        # Use the overall mean (consistent with kpi_extractor which computes mean
        # across all samples, not mean-of-node-means; also consistent with the
        # std that is computed around rssi_mean_val, not around mean-of-means)
        global_rssi = round(rssi_mean_val, 2) if rssi_mean_val is not None else round(sum(rssi_means.values()) / len(rssi_means), 2)
